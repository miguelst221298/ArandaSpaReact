...
export default function Paginacion(props: paginacionProps) {
    ...
    useEffect(() => {
        const paginAnteriorHabilitada = props.paginaActual > 1;
        const paginaAnterior = props.paginaActual - 1;
        const links: modeloLink[] = [];

        links.push({ texto: 'Anterior', habilitado: paginAnteriorHabilitada, pagina: paginaAnterior, activo: false });


        for (let i = 1; i <= props.cantidadTotalDePaginas; i++) {
            if (i >= props.paginaActual - props.radio && i <= props.paginaActual + props.radio) {
                links.push({ texto: `${i}`, habilitado: true, pagina: i, activo: props.paginaActual === i });
            }
        }
        ...
        links.push({ texto: 'Siguiente', habilitado: paginaSiguienteHabilitada, pagina: paginaSiguiente, activo: false });
    ...
    return (<>
        <nav >
            <ul className="pagination justify-content-cent">
                {listadoLinks.map((link) => {

                   return <li className={`page-item cursor ${obtenerClase(link)}`}
                        key={link.texto}
